...
Open-source replacement for Microsoft's retired Intune Device Offboarding Agent. Identifies devices that have gone stale across both Intune (last sync) and Entra ID (last sign-in), correlates them, and offboards them via Intune retire after typed confirmation.
...
5. Generates a plain-language rationale via the local LLM, honoring any custom admin instructions.
6. Pauses for typed confirmation (`OFFBOARD N DEVICES`).
7. On confirm: issues `POST /deviceManagement/managedDevices/{id}/retire` for each candidate.
