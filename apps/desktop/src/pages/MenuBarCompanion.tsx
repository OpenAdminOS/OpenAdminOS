  ...
  return (
    <main className="companion-shell flex h-full w-full flex-col overflow-hidden text-[var(--color-text)]">
      <section className="relative z-10 flex min-h-0 flex-1 flex-col gap-2.5 p-3">
        ...
        {notice && (
          <div className="animate-companion-rise rounded-lg bg-[var(--color-info-soft)] px-3 py-2 text-[11.5px] leading-relaxed text-[var(--color-info)] ring-1 ring-[var(--color-info)]/25">
            {notice}
          </div>
        )}
  ...
  return (
    <div className="grid gap-1.5">
      {snapshot.attention.slice(0, 2).map((item) => (
        <button
          key={item.id}
          className={`animate-companion-rise w-full rounded-lg px-3 py-2 text-left ring-1 transition-transform duration-200 hover:translateY-[-1px] ${item.severity === "danger" ? "bg-[var(--color-danger-soft)] text-[var(--color-danger)] ring-[var(--color-danger)]/25" : "bg-[var(--color-warning-soft)] text-[var(--color-warning)] ring-[var(--color-warning)]/25"}`}
          onClick={() => item.actionRoute && onOpen(item.actionRoute)}
        >
          <span className="block text-[11.5px] font-semibold">{item.title}</span>
          <span className="mt-0.5 line-clamp-2 block text-[10.5px] leading-relaxed opacity-85">
            {item.body}
          </span>
        </button>
      ))}
...
function AskPanel({
  ...
  return (
    <div className="companion-panel animate-companion-rise p-2.5">
      ...
      {chat.phase !== "idle" && (
        <div className={`mb-2 max-h-[132px] overflow-auto rounded-lg px-3 py-2.5 ring-1 ${chat.phase === "failed" ? "bg-[var(--color-danger-soft)] text-[var(--color-danger)] ring-[var(--color-danger)]/25" : "bg-black/12 text-[var(--color-text-soft)] ring-white/[0.05]"}`}>
