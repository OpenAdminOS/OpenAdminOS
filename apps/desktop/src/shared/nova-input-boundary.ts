...
export function novaInputWaitMs(text: string, sinceTranscript: number, sinceVoice: number): number {
  ...
  const unfinished = !isNovaIntroduction(text) && (questionPrefix || /\b(?:am|is|are|was|were|be|been|being|do|does|did|have|has|had|can|could|would|should|will|may|might|must|and|or|but|because|if|when|which|whose|with|without|of|to|for|from|on|in|at|by|the|a|an|my|your|our|not)\s*[,.…-]*$/i.test(question));
