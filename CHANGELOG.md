...
- Point-in-time and cross-tenant configuration compare engines: diff a tenant against itself at two moments (with an explicit warning when retention limits the window), or diff two tenants by matching objects on unique display names, never guessing on duplicates, and excluding tenant-specific fields and, by default, assignments.
