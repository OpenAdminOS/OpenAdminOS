...
- ui: Agent Hub renders a `Requires Entra ID P1/P2` pill next to the read/write and category pills. Tone is `warning` when the active tenant doesn't satisfy the requirement (with a tooltip explaining the gap), muted otherwise. Status strip's tenant chip now shows the detected tier ("tenant: openadminos.example  Entra P2").
