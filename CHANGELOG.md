...
- Use teammate naming consistently, move the detailed briefing into Review inbox, and keep room labels readable with a full-name roster and fixed evidence strip.
...
- Settings gains a Privacy control for usage telemetry (the exact payload preview, a test ping, and the collector-not-configured notice) and a documentation-grounding status panel that shows the installed index date, chunk count, and embedding model, or an honest not-grounded state, with a refresh action.
