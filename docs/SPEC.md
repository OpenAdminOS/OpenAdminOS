...
### openadminos.com/benchmarks (decided 2026-08-31)

The training status page and the `training.openadminos.com` subdomain were
removed on 2026-08-31: a live view of our own pipeline served us, not the
admin evaluating the product. It is replaced by a `/benchmarks` route on the
main site, linked from the landing page and the primary navigation.

- **Purpose**: show an admin that a local model is sufficient for their daily
  work. OpenAdmin 8B is measured against hosted frontier models on identical
  Microsoft 365 administration tasks, scored mechanically.
- **Data**: the page renders exclusively from
  `web/src/data/benchmarks/public-data.json`, generated by
  `model/site-benchmarks/export-benchmark-data.mjs` from scored runs in
  `model/eval/results` and validated at build time. No hand-transcribed
  numbers, and no build-time fetch from a sibling folder.
...
- The design-history mockups (16/17) remain; they document a direction that
...
installed-build audio acceptance; see `tasks/nova-audio-2026-09-16.md`.
